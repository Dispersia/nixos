...
__QR_plugin_versionMinorMinor__ = "1"
...
def InstallQuadRemesherEngineIfNeeded(op, context, enginePath):
    if not os.path.exists(enginePath):
        ...
        if isMacOSX:
            ...
            urlList.append("http://exoside.com/quadremesherdata/download_engine.php?qrVer="+__QR_plugin_version__+"&qrVerMM="+__QR_plugin_versionMinorMinor__+"&os=macOS&ha=Blender&haVer="+getHostAppVer())
            urlList.append("http://exoside.com/quadremesherdata/quad_remesher_engine_"+__QR_plugin_version__+"_macOS.zip")
        elif isLinux :
            urlList.append("http://exoside.com/quadremesherdata/download_engine.php?qrVer="+__QR_plugin_version__+"&qrVerMM="+__QR_plugin_versionMinorMinor__+"&os=linux&ha=Blender&haVer="+getHostAppVer())
            urlList.append("http://exoside.com/quadremesherdata/quad_remesher_engine_"+__QR_plugin_version__+"_linux.zip")
        else:
            urlList.append("https://exoside.com/quadremesherdata/download_engine.php?qrVer="+__QR_plugin_version__+"&qrVerMM="+__QR_plugin_versionMinorMinor__+"&os=win&ha=Blender&haVer="+getHostAppVer()) 
        ...
        try:
            if isMacOSX:
                ...
                # using ditto to keep signature and notarization
                #print('ditto -x -k "%s" "%s"' % (zip_file_name, engineFolder))
                os.system('ditto -x -k "%s" "%s"' % (zip_file_name, engineFolder))
...
def doRemeshing_Finish(theOp, context) :
    ...
    if getattr(props, 'hide_input'):
        # NB: hide_set() controls the Visibility (eye icon);   'hide_viewport' controls the 'Disabled in Viewport' (Monitor icon)    'hide_render' controls the 'Disabled in Render' (Camera icon)    
        inputObject.hide_set(state=True)   # NB: 2.8 only, 2.7 must use 'hide'
        # new 17 june 2025: 1.4 (just after 1st beta)
        try:
            inputObject.hide_viewport = True
            inputObject.hide_render = True
        except Exception:
            print("no hide_render")
